            ...
            if(error && error.sqlMessage && !error.sqlMessage.includes('database exists')){
                ...
                con.query("use "+config.db.dbName+";", function(error, results, fields){
                    if(error && error.sqlMessage && !error.sqlMessage.includes('database exists')){
                        ...
                        con.query("CREATE table "+config.db.userList+" (f_name varchar(255), l_name varchar(255), username varchar(255) NOT NULL UNIQUE, password varchar(255), email varchar(255), role varchar(255));", function(error, results, fields){
